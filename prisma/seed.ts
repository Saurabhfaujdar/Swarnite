...
async function main() {
  ...
  const mainBranch = await prisma.branch.upsert({
    ...
    create: {
      ...
      isMaster: true, // This should be true for the master branch
    },
  });
  ...
  console.log(
    "  ✅ Metal types created:",
    metalTypeData.map((m) => m.name).join(", "),
  );

  const goldMetal = await prisma.metalType.findUnique({
    where: { name: "Gold" },
  });
  const silverMetal = await prisma.metalType.findUnique({
    where: { name: "Silver" },
  });
  const platinumMetal = await prisma.metalType.findUnique({
    where: { name: "Platinum" },
  });
  ...
  const necklaceGroup = await prisma.itemGroup.findUnique({
    where: { code: "NEC" },
  });
  const chainGroup = await prisma.itemGroup.findUnique({
    where: { code: "CHN" },
  });
  const bangleGroup = await prisma.itemGroup.findUnique({
    where: { code: "BNG" },
  });
  const ringGroup = await prisma.itemGroup.findUnique({
    where: { code: "RNG" },
  });
  const earringGroup = await prisma.itemGroup.findUnique({
    where: { code: "EAR" },
  });
  const pendantGroup = await prisma.itemGroup.findUnique({
    where: { code: "PND" },
  });
  const mangalsutraGroup = await prisma.itemGroup.findUnique({
    where: { code: "MNG" },
  });
  const coinGroup = await prisma.itemGroup.findUnique({
    where: { code: "CON" },
  });
  const sBangleGroup = await prisma.itemGroup.findUnique({
    where: { code: "SBG" },
  });
  const sCoinGroup = await prisma.itemGroup.findUnique({
    where: { code: "SCN" },
  });
  ...
  const itemDefs = [
    {
      ...
      code: "GN22",
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: necklaceGroup!.id,
    },
    {
      ...
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: chainGroup!.id,
    },
    {
      name: "Gold Bangle 22KT",
      code: "GB22",
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: bangleGroup!.id,
    },
    {
      name: "Gold Ring 22KT",
      code: "GR22",
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: ringGroup!.id,
    },
    {
      name: "Gold Earring 22KT",
      code: "GE22",
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: earringGroup!.id,
    },
    {
      ...
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: pendantGroup!.id,
    },
    {
      name: "Gold Mangalsutra 22KT",
      code: "GM22",
      metalTypeId: goldMetal!.id,
      purityId: gold916!.id,
      itemGroupId: mangalsutraGroup!.id,
    },
    {
      name: "Gold Coin 24KT",
      code: "GCN24",
  ...
  const sampleCustomers = [
    ...
    {
      name: "Gold Supplier Trading Co.",
      mobile: "9876543220",
      type: "SUPPLIER" as const,
      city: "Ahmedabad",
    },
    {
      name: "Silver World Pvt Ltd",
      mobile: "9876543221",
      type: "SUPPLIER" as const,
      city: "Rajkot",
    },
